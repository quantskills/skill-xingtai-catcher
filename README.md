...
- 使用固定模板：强趋势延续、底部反转这类服务器模板可以直接筛选，不需要每次重新画图。
...
默认命令会输出可直接给用户看的摘要。不要把 `System (untrusted)`、`Exec completed`、原始 JSON 或命令执行日志贴给用户；如果需要调试，才在命令最后加 `--json`。
...
| 周期 | 1d、60m | 未说明时先询问；说“默认”时用 1d |
| 数据长度 | 30、60、120 BAR | 未说明时先询问；说“默认”时用 120 |
...
- “60分钟”“小时线”“短线” -> `60m`
- “多给几个” -> 最多 `Top10`
- 只说“找 W底”“找 M头”“找趋势”但没有说周期和 BAR -> 先问用户，不直接搜索
- “强趋势延续”“底部反转”这类标准筛选 -> 优先推荐今日雷达模板，不要求用户重新画图
- 用户要画 W 底/M 顶 -> 提醒先画一段前置下跌/上涨
